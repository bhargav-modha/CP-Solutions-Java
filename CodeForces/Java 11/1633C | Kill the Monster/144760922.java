...
public class C_Kill_the_Monster {
 public static void main(String[] args) {
  ...
  while (t-- > 0) {
   ...
   long k_coins = sc.nextLong();
   long w_increase = sc.nextLong();
   long a_increase = sc.nextLong();
 
   boolean flag = false;
   for (int i = 0; i <= k_coins; i++) {
    long count1 = (Hm - 1) / (Dc + i * w_increase) + 1;
    long count2 = ((Hc + (k_coins - i) * a_increase) - 1) / Dm + 1;
