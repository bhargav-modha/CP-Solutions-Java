...
public class F_Balance_between_offline_classes {
 public static void main(String[] args) {
  MyScanner sc = new MyScanner();
  out = new PrintWriter(new BufferedOutputStream(System.out));
  int n = sc.nextInt();
  long arr[] = sc.readLongArray(n); //reading full array
  sort(arr);
 
  int ans = 0, low = 0;
 
  // Using outer loop to iterate over 1 to n
 
  for (int i = 0; i < n; ++i) {
 
   // Using inner loop to Maximize lower index till it is less than 5
   while (arr[i] - arr[low] > 5)
    low++; //incrementing lower index
   ans = Math.max(ans, i - low + 1); //Checking for Max till now
  }
  out.println(ans);
  out.close();
 }
