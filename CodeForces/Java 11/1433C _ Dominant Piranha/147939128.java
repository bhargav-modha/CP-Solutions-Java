...
public class C_Dominant_Piranha {
 public static void main(String[] args) {
  ...
  int t = sc.nextInt();
  while (t-- > 0) {
   int n = sc.nextInt(), piranha[] = sc.readArray(n), max = Integer.MIN_VALUE, index = -1;
 
   for (int i = 0; i < n; i++)
    max = Math.max(max, piranha[i]);
 
   for (int i = 0; i < n; i++)
    if (piranha[i] == max && (i > 0 && piranha[i - 1] < max || i < n - 1 && piranha[i + 1] < max))
     index = i + 1;
 
   out.println(index >= 0 ? index : "-1");
  }
  out.close();
 }
