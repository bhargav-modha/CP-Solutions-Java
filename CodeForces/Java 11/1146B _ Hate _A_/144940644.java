...
public class B_Hate_A_ {
 public static void main(String[] args) {
  MyScanner sc = new MyScanner();
  out = new PrintWriter(new BufferedOutputStream(System.out));
  String s = sc.next();
  StringBuilder withoutA = new StringBuilder();
 
  for (char x : s.toCharArray())
   if (x != 'a')
    withoutA.append(x);
 
  String SwithoutA = withoutA.toString();
  String first = SwithoutA.substring(0, SwithoutA.length() / 2);
  String second = SwithoutA.substring(SwithoutA.length() / 2);
  ...
  if (!first.equals(second))
   ...
   out.println(s.substring(0, s.length() - second.length()));
  out.close();
 }
