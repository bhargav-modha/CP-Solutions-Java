...
public class B_Twogram {
 public static void main(String[] args) {
  ...
  for (int i = 0; i < n - 1; i++) {
   String subset = full.substring(i, i + 2);
   if (occurMap.containsKey(subset)) {
    int temp = occurMap.get(subset);
    occurMap.replace(subset, temp, ++temp);
   } else
    occurMap.put(subset, 1);
  }
  ...
  out.println(String.valueOf(maxEntry).substring(0, 2));
  out.close();
 }
