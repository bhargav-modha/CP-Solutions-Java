...
public class A_Romaji {
 public static void main(String[] args) {
  MyScanner sc = new MyScanner();
  out = new PrintWriter(new BufferedOutputStream(System.out));
  String str = sc.next();
  for (int i = 0; i < str.length(); i++) {
   if (!isVowel(str.charAt(i)) && !isN(str.charAt(i))) {
    if (i == str.length() - 1 || !isVowel(str.charAt(i + 1))) {
     out.println("NO");
     out.close();
     return;
    }
 ...
 static boolean isVowel(char c) {
  return "aouie".indexOf(c) != -1;
 }
 
 static boolean isN(char c) {
  return c == 'n';
 }
