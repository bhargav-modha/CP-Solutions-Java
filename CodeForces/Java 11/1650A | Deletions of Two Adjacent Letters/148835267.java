...
public class A_Deletions_of_Two_Adjacent_Letters {
 public static void main(String[] args) {
  ...
  while (t-- > 0) {
   ...
   char needed = sc.next().charAt(0);
 
   if (!s.contains(String.valueOf(needed)))
    ans = "NO";
   else {
    StringBuilder sb = new StringBuilder(s);
    boolean flag = true;
    for (int i = 0; i < sb.length(); i++) {
     //out.println((i-1)%2 +" "+ (sb.length()-(sb.length()-(i-1))));
     if(sb.charAt(i) == needed && ((i-1)%2 != 0 && (sb.length()-(sb.length()-(i-1)))%2 != 0))
     {
      ans = "YES";
      flag = false;
      break;
