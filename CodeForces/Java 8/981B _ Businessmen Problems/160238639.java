...
public class B_Businessmen_Problems {
 public static void main(String[] args) {
  MyScanner sc = new MyScanner();
  out = new PrintWriter(new BufferedOutputStream(System.out));
  int n = sc.nextInt();
  Pair[] list1 = new Pair[n];
  for (int i = 0; i < list1.length; i++) list1[i] = new Pair(sc.nextLong(), sc.nextLong());
 
  int m = sc.nextInt();
  Pair[] list2 = new Pair[m];
  for (int i = 0; i < list2.length; i++) list2[i] = new Pair(sc.nextLong(), sc.nextLong());
 
  Pair[] fPairs = new Pair[n + m];
  for (int i = 0; i < n; i++) fPairs[i] = list1[i];
  for (int i = 0; i < m; i++) fPairs[n + i] = list2[i];
 
  Pair.sortbyy(fPairs);
 
  HashMap<Long, Long> map = new HashMap<>();
  for (int i = 0; i < fPairs.length; i++) map.put(fPairs[i].x, fPairs[i].y);
 
  long total = 0;
  for (Map.Entry<Long, Long> entry : map.entrySet()) total += entry.getValue();
