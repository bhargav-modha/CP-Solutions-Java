...
public class C_Restoring_the_Duration_of_Tasks {
 public static void main(String[] args) {
  ...
  while (t-- > 0) {
   int n = sc.nextInt();
   long[] arrivaltime = sc.readLongArray(n), completiontime = sc.readLongArray(n);
 
   out.print(completiontime[0] - arrivaltime[0] + " ");
   for (int i = 1; i < n; i++) {
    if (arrivaltime[i] < completiontime[i - 1]) {
     out.print(completiontime[i] - completiontime[i - 1] + " ");
    } else
     out.print(completiontime[i] - arrivaltime[i] + " ");
   }
