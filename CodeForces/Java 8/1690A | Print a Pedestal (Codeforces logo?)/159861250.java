...
public class A_Print_a_Pedestal_Codeforces_logo {
 public static void main(String[] args) {
  MyScanner sc = new MyScanner();
  out = new PrintWriter(new BufferedOutputStream(System.out));
  int t = sc.nextInt();
  while (t-- > 0) {
   int n = sc.nextInt(), m = (n-3)/3;
   if(n%3 == 0) out.println((m+1) + " " + (m+2) + " " + (m));
   if(n%3 == 1) out.println((m+1) + " " + (m+3) + " " + (m));
   if(n%3 == 2) out.println((m+2) + " " + (m+3) + " " + (m));
  }
  out.close();
 }
