...
public class A_Little_Elephant_and_Interval {
 ...
 public static void solve(BigInteger l, BigInteger r) {
  out.println(getCount(r).subtract(getCount(l.subtract(BigInteger.ONE))));
 }
 
 public static BigInteger getCount(BigInteger x) {
  if (x.compareTo(new BigInteger("10")) < 0)
   return x;
 
  BigInteger tens = x.divide(new BigInteger("10"));
  BigInteger res = tens.add(new BigInteger("9"));
 
  BigInteger firstDigit = getFirstDigit(x);
  BigInteger lastDigit = x.mod(new BigInteger("10"));
 
  if (lastDigit.compareTo(firstDigit) < 0)
   res = res.subtract(BigInteger.ONE);
 
  return res;
 }
 
 public static BigInteger getFirstDigit(BigInteger x) {
  while (x.compareTo(new BigInteger("10")) >= 0)
