...
public class B_Combination {
 public static void main(String[] args) {
  ...
  int n = sc.nextInt();
  ArrayList<Pair> cards = new ArrayList<>(), cards2 = new ArrayList<>();
  for (int i = 0; i < n; i++) {
   Pair temp = new Pair(sc.nextInt(), sc.nextInt());
   if (temp.y > 0)
    cards.add(temp);
   else
    cards2.add(temp);
  ...
  if (cards.isEmpty()) {
   out.println(cards2.get(0).x);
  } else {
   int counter = 1, points = 0;
   do {
    points += cards.get(0).x;
    counter += cards.get(0).y;
    counter--;
    cards.remove(0);
   } while (!cards.isEmpty());
 
   if (counter > 0) {
    while (counter > 0 && !cards2.isEmpty()) {
     points += cards2.get(0).x;
     counter--;
     cards2.remove(0);
    }
   }
