...
public class D_XSum {
 public static void main(String[] args) {
  ...
  int t = sc.nextInt();
  while (t-- > 0) {
   int n = sc.nextInt(), m = sc.nextInt(), arr[][] = new int[n][m], arr_1d[] = new int[n * m];
   int idx = 0;
   for (int i = 0; i < n; i++) {
    for (int j = 0; j < m; j++) {
     arr[i][j] = sc.nextInt();
   ...
   for (int i = 0; i < n; i++)
    for (int j = 0; j < m; j++)
     tset.add(diagonals(arr_1d, n, m, (idx++)) - arr[i][j]);
   out.println(tset.last());
  }
  out.close();
 }
 
 public static int diagonals(int arr[], int h, int w, int n) {
  int sum = 0;
  int nRow = (int) Math.floor(n / w), nCol = n % w;
 
  // ArrayList<Integer> LTR = new ArrayList<>();
  for (int r = nRow - Math.min(nRow, nCol), c = nCol - Math.min(nRow, nCol); r < h && c < w; r++, c++) {
   // LTR.add(arr[r * w + c]);
   sum += arr[r * w + c];
  }
 
  // ArrayList<Integer> RTL = new ArrayList<>();
  for (int r = nRow - Math.min(nRow, w - nCol - 1), c = nCol + Math.min(nRow, w - nCol - 1); r < h
    && 0 <= c; r++, c--) {
   // RTL.add(arr[r * w + c]);
   sum += arr[r * w + c];
  }
