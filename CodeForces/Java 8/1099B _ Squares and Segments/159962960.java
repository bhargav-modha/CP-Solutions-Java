...
public class B_Squares_and_Segments {
 public static void main(String[] args) {
  MyScanner sc = new MyScanner();
  out = new PrintWriter(new BufferedOutputStream(System.out));
  int sq = sc.nextInt();
  
  if(sq == 1) out.println(2);
  else if(sq == 2) out.println(3);
  else{
   int cur = (int)Math.ceil(Math.sqrt(sq));
   if(cur * (cur - 1) >= sq) out.println(2*cur - 1);
   else out.println(2*cur);
  }
  out.close();
 }
 ...
 public static int lower_bound(ArrayList<Integer> arr, int x) {
 ...
 public static int upper_bound(ArrayList<Integer> arr, int x) {
  int low = 0, high = arr.size() - 1, mid = -1;
  int ans = arr.size();
  while (low < high) {
   mid = (low + high) / 2;
