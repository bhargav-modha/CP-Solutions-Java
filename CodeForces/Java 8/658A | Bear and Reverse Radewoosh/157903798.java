...
public class A_Bear_and_Reverse_Radewoosh {
 public static void main(String[] args) {
  MyScanner sc = new MyScanner();
  out = new PrintWriter(new BufferedOutputStream(System.out));
  int n = sc.nextInt(), c = sc.nextInt(), scores[] = sc.readArray(n), minutes[] = sc.readArray(n), f_score1 = 0,
    f_score2 = 0, limak_min[] = new int[n], rade_min[] = new int[n], score2[] = new int[n];
  limak_min[0] = minutes[0];
  rade_min[0] = minutes[n - 1];
  for (int i = 1; i < minutes.length; i++)
   limak_min[i] = limak_min[i - 1] + minutes[i];
 
  int idx = 1;
  for (int i = n - 2; i >= 0; i--)
   rade_min[idx++] = rade_min[idx - 2] + minutes[i];
 
  for (int i = 0; i < score2.length; i++)
   score2[i] = scores[n - i - 1];
 
  for (int j = 0; j < minutes.length; j++) {
   f_score1 += max(0, scores[j] - c * limak_min[j]);
   f_score2 += max(0, score2[j] - c * rade_min[j]);
  }
 
  out.println(f_score1 == f_score2 ? "Tie" : (f_score1 > f_score2) ? "Limak" : "Radewoosh");
  out.close();
 }
