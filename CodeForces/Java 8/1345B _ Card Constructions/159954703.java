...
public class B_Card_Constructions {
 public static void main(String[] args) {
  ...
  cards.add(2L);
  for (long i = 5; i < 1000000; i += 3)
   cards.add(cards.get(cards.size() - 1) + i);
  ...
  while (t-- > 0) {
   ...
   while (true) {
    int temp = lower_bound(cards, sticks);
    if (temp >= 0) {
     sticks -= cards.get(temp);
     count++;
    } else
     break;
   }
 ...
 public static int lower_bound(ArrayList arr, long x) {
  int low = 0, high = arr.size() - 1, mid = -1;
  int ans = -1;
  while (low <= high) {
   mid = (low + high) / 2;
   if (((long) arr.get(mid)) > x) {
    high = mid - 1;
   } else {
    ans = mid;
    low = mid + 1;
