...
public class D_Two_Strings_Swaps {
 public static void main(String[] args) {
  ...
  for (int i = 0; i < n / 2; i++) {
   int right = n - 1 - i;
   if (a[right] == a[i] && b[right] == b[i]) continue;
   if (a[i] == b[i] && a[right] == b[right]) continue;
   if (a[right] == b[i] && a[i] == b[right]) continue;
   if (b[i] != b[right]) {
    if (b[i] != a[i] && b[right] != a[i] && b[i] != a[right] && b[right] != a[right])
     ans += 2;
    else 
     ans++;
   } else
