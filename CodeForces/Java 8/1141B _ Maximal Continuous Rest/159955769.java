...
public class B_Maximal_Continuous_Rest {
 public static void main(String[] args) {
  MyScanner sc = new MyScanner();
  out = new PrintWriter(new BufferedOutputStream(System.out));
  int n = sc.nextInt(), arr[] = sc.readArray(n), count = 0, max = Integer.MIN_VALUE;
 
  for (int i = 0; i < arr.length*(2.5); i++) {
   if (arr[(i)%n] == 1)
    count++;
   else
    count = 0;
  ...
  out.println(n>=2?max:count(arr, 1));
  out.close();
 }
