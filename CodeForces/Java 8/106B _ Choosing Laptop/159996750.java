...
public class B_Choosing_Laptop {
 public static void main(String[] args) {
  ...
  for (int i = 0; i < n; i++) {
   Laptop temp = new Laptop(sc.nextInt(), sc.nextInt(), sc.nextInt(), sc.nextInt(), (i + 1));
   list.add(temp);
  }
 
  ArrayList<Laptop> f_list = new ArrayList<>();
  for (int i = 0; i < n; i++) {
   boolean bad = false;
   for (int j = 0; j < n; j++) {
    if (list.get(i).Speed < list.get(j).Speed && list.get(i).HDD < list.get(j).HDD &&
      list.get(i).RAM < list.get(j).RAM)
     bad = true;
   }
   if (!bad)
  ...
  //out.println(f_list);
  Laptop.sortbyPrice(f_list);
  out.println(f_list.get(0).index);
  out.close();
 }
 ...
 public static class Laptop {
  ...
  Laptop(int Speed, int RAM, int HDD, int price, int index) {
   this.Speed = Speed;
   this.RAM = RAM;
   this.HDD = HDD;
   this.price = price;
  ...
  static void sortbyPrice(ArrayList<Laptop> al) {
