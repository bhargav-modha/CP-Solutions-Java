...
public class C_Circular_Local_MiniMax {
  ...
  while (t-- > 0) {
   ...
   if (n % 2 != 0)
    ...
    for (int i = 0; i < arr.length / 2; i++) {
     seq.add(arr[i]);
     seq.add(arr[arr.length / 2 + i]);
    }
    boolean flag = true;
    for (int i = 1; i < seq.size() - 1; i++) {
     if (!((seq.get(i - 1) < seq.get(i) && seq.get(i) > seq.get(i + 1)) ||
       (seq.get(i - 1) > seq.get(i) && seq.get(i) < seq.get(i + 1)))) {
      out.println("NO");
      flag = false;
      break;
     }
