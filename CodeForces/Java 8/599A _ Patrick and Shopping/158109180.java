...
public class A_Patrick_and_Shopping {
 public static void main(String[] args) {
  MyScanner sc = new MyScanner();
  out = new PrintWriter(new BufferedOutputStream(System.out));
  int d1 = sc.nextInt(), d2 = sc.nextInt(), d3 = sc.nextInt(), ans = 0;
  ans += min(d1, d2);
  if (d1 < d2)
   ans += min(min((d1 + 2 * d2), (d3 + d2)), 2 * d3 + d1);
  else
   ans += min(min((d2 + 2 * d1), (d3 + d1)), 2 * d3 + d2);
  out.println(ans);
  out.close();
 }
