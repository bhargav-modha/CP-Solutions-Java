...
public class B_Unlucky_Ticket {
 public static void main(String[] args) {
  ...
  ArrayList<Integer> list_first = new ArrayList<>(), list_second = new ArrayList<>();
  for (int i = 0; i < n; i++)
   list_first.add(Integer.parseInt(String.valueOf(num[i])));
  for (int i = n; i < 2 * n; i++)
   list_second.add(Integer.parseInt(String.valueOf(num[i])));
 
  Collections.sort(list_first);
  Collections.sort(list_second);
 
  boolean flag = false;
  if (list_first.get(0) > list_second.get(0)) {
   for (int i = 1; i < n; i++) {
    if (list_first.get(i) <= list_second.get(i)) {
     out.println("NO");
     flag = true;
     break;
    }
   ...
   if (!flag)
    out.println("YES");
  } else if (list_first.get(0) < list_second.get(0)) {
   for (int i = 1; i < n; i++) {
    if (list_first.get(i) >= list_second.get(i)) {
     out.println("NO");
     flag = true;
